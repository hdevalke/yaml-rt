...
tagged enums. Unit structs overlay null scalars, while multi-field tuple structs
overlay fixed-length sequences and patch their elements positionally.
Same-variant enum payloads use the existing scalar, sequence, and mapping
editors; variant changes replace only the selected node. Unknown source entries
remain untouched by default.
